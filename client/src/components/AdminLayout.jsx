...
const AdminLayout = () => {
  ...
  return (
    <div className="flex h-full min-h-[80vh] flex-col md:flex-row shadow-sm border border-accent-gold/10 bg-surface">
      {/* Mobile Toggle Button */}
      <div className="md:hidden bg-bg-base border-b border-accent-gold/10 p-4 flex justify-between items-center z-20">
        <span className="font-bold tracking-widest text-accent-gold uppercase text-sm">Admin Panel</span>
      ...
      <aside
        className={`${isSidebarOpen ? 'block' : 'hidden'
          } md:block w-full md:w-64 bg-[#050505] border-r border-accent-gold/10 text-text-secondary flex-shrink-0 transition-all duration-300 z-10`}
      >
        <div className="p-6 hidden md:block border-b border-accent-gold/10">
          <h2 className="text-sm font-extrabold tracking-widest text-accent-gold uppercase">Back Office</h2>
        </div>
        <nav className="p-4 space-y-1">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 min-h-12 transition-all duration-200 font-medium ${isActive
                  ? 'border-l-2 border-accent-gold text-accent-gold bg-[#111111]'
                  : 'hover:text-accent-gold text-text-secondary hover:bg-[#0A0A0A]'
                }`
              }
            >
      ...
      {/* Main Content Area */}
      <main className="flex-1 p-3 pb-24 md:pb-8 md:p-8 overflow-y-auto bg-bg-base">
        <Outlet />
      </main>
    </div>
  );
