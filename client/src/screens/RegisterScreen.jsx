  ...
  return (
    <div className="flex min-h-[85vh] rounded-3xl overflow-hidden border border-accent-gold/20 shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-fade-in mb-8">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-surface items-center justify-center overflow-hidden group">
        <img 
          src="https://images.unsplash.com/photo-1599643477874-5c866f466cb1?q=80&w=1400&auto=format&fit=crop" 
          alt="Join Eclipsera Premium" 
        ...
        <div className="relative z-10 text-center px-12 pb-12 mt-auto">
          <h2 className="text-4xl font-serif text-accent-gold mb-4 tracking-wider">Join The Collective</h2>
          <p className="text-text-primary/90 text-lg font-light leading-relaxed">Create an account to track your orders, save your wishlist, and receive early access to our most exclusive drops.</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 bg-surface/80 backdrop-blur-xl relative">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <h1 className="text-3xl font-serif font-extrabold text-text-primary mb-8 text-center tracking-wide">Create Account</h1>
          ...
          <form onSubmit={handleSubmit(submitHandler)} className="space-y-5">
            <div>
              ...
              <input
                type="text"
                className={`w-full px-5 py-4 rounded-xl bg-bg-base border ${errors.name ? 'border-red-500 focus:ring-red-500' : 'border-accent-gold/30 focus:ring-accent-gold'} focus:ring-2 outline-none transition-all duration-300 text-text-primary`}
            ...
            <div>
              ...
              <input
                type="password"
                className={`w-full px-5 py-4 rounded-xl bg-bg-base border ${errors.confirmPassword ? 'border-red-500 focus:ring-red-500' : 'border-accent-gold/30 focus:ring-accent-gold'} focus:ring-2 outline-none transition-all duration-300 text-text-primary`}
                placeholder="Confirm password"
                {...register('confirmPassword')}
              />
              {errors.confirmPassword && <p className="text-red-500 text-xs mt-2 font-medium">{errors.confirmPassword.message}</p>}
            </div>
          ...
          <div className="mt-10 text-center text-text-secondary text-sm">
            ...
            <Link to={redirect !== '/' ? `/login?redirect=${redirect}` : '/login'} className="text-accent-gold font-bold hover:underline transition-all">
              Sign In Here
            </Link>
          </div>
        </div>
