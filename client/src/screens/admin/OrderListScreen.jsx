              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="text-lg font-bold text-accent-gold">Customer Details</h3>
                    {selectedOrder.shippingAddress?.phone && (
                      <a 
                        href={`https://wa.me/${selectedOrder.shippingAddress.phone}?text=${encodeURIComponent(`Hi ${selectedOrder.user?.name || ''}, this is Eclipsera. We are contacting you regarding your order of ${selectedOrder.orderItems?.[0]?.name || 'items'}.`)}`}
                        target="_blank"
                        rel="noreferrer"
                        className="bg-[#25D366] hover:bg-[#128C7E] text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-md transition-colors"
                      >
