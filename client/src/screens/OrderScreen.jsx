...
const OrderScreen = () => {
  ...
  return (
    <div className="py-12 animate-fade-in max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <h1 className="text-2xl md:text-3xl font-serif font-extrabold text-text-primary mb-8">Order Details: <span className="text-accent-gold font-sans text-xl">{order.orderItems?.[0]?.name || 'Custom Item'} {order.orderItems?.length > 1 ? `(+${order.orderItems.length - 1} items)` : ''}</span></h1>
          ...
          <div className="bg-transparent rounded-3xl p-8 border border-accent-gold/20 shadow-sm sticky top-8">
            ...
            {userInfo && userInfo.isAdmin && (
              <div className="mt-4">
                {order.shippingAddress?.phone && (
                  <a
                    href={`https://wa.me/${order.shippingAddress.phone}?text=${encodeURIComponent(`Hi ${order.user?.name || order.shippingAddress?.name || ''}, this is Eclipsera. Your order of ${order.orderItems?.[0]?.name || 'items'} shipping to ${order.shippingAddress.city} is currently ${order.isDelivered ? 'Delivered' : order.fulfillmentStatus || 'being processed'}.`)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="w-full mb-3 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold py-4 min-h-12 rounded-xl shadow-md transition-all hover:shadow-lg flex items-center justify-center gap-2"
                  >
